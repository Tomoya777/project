...
task_datatype = [
  ...
  ("estimated_time", "int"), #課題の推定時間(int). scorm抽出時は0.
  ("progress", "int")] #課題の完成度. scorm抽出時は0.

def Scomblogin (USER, PASSW):
  TIME_OUT = 10 #タイムアウト時間
  ...
  driver = webdriver.Chrome(options=options, executable_path= "chromedriver.exe") #chromeドライバの起動
...
def Scombkadai (driver, wait,siteuser):
...
USER="" #デバック用ユーザー名
PASSW="" #デバック用パスワード
SITE_USER = "al19063" #デバック用ユーザー名  
driver, wait = Scomblogin(USER, PASSW)
task_array = Scombkadai(driver, wait, SITE_USER)

f = open('test1.txt', 'w', encoding="utf-8_sig") #テスト用に出力
f.write(str(task_array))
f.close()
